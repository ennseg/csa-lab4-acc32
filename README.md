...
- **Регистры** (DataPath): `AC` (аккумулятор), `AC_SHADOW` (теневой,
  ...
  возвратов), `SR` (флаги: N, Z, V, C, IE), `AC_AR`/`ACS_AR` и биты
...
Control Unit (hardwired): Decoder + регистр состояния (FETCH/DECODE/EXECUTE)
...
- `SR` (регистр флагов `IE|N|Z|V|C`) — вход из АЛУ (N,Z,V,C) и от CU (IE),
...
- **Decoder** — по входам формирует все управляющие сигналы. Входы: опкод
  ...
  `eq_ac`/`eq_acs` (для суперскалярных решений). Выходы (группами):
  `latch_signals` (защёлки регистров), `sel_signals` (селекторы MUX),
  `alu_op` (код операции АЛУ), `mem w/r` (чтение/запись памяти), `I/O`,
  `check_signals` (управление битами валидности).
- **State** (регистр состояния) — хранит фазу цикла (FETCH / DECODE /
  EXECUTE) и номер шага исполнения. Определяет, какой микрошаг плана
  выполняется на текущем такте; декодер использует его вместе с опкодом.
...
(реализм UART: символ просто попадает в порт, без аппаратного буфера).
...
3. сохраняет на аппаратный стек возвратов **три ячейки** контекста (стек
   ...
   - регистр флагов `SR`, упакованный в одно слово (`IE|N|Z|V|C`);
...
`IE|N|Z|V|C`), затем аккумулятор `AC` (через АЛУ `PASS_DR`, без обновления
